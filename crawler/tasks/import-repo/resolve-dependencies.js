  ...
  return _.flow(
    ...
    _.map(([packageName, version]) => {
      if (!packages[packageName]) {
        throw new Error(`Dependency '${packageName}' doesn't exist`)
      }
...
function getAbsoluteVersion (package, version) {
  ...
  if (!match) {
    throw new Error(`Dependency '${package.name}' has an invalid version format '${version}'`)
  }
  ...
  }, versions.reverse()) // start with latest version

  if (!absoluteVersion) {
    throw new Error(`Dependency '${package.name}' has no matching version for '${version}'`)
  }
